...
void LoadInput(InputCondition ICond, InputMaterial IMaterial, InputProperty IProp)
{
  ...
  IMaterial->aVol = std::pow(IMaterial->aLat, 3) / Na; // Atomic volume (m3)
  ...
  IMaterial->D[0] = (1.49E-4) * std::exp(-234000.0 / (8.314 * ICond->Temp));  // Mn thermal diffusion coefficient, m2/s
  IMaterial->D[1] = (1.4E-4) * std::exp(-245765.0 / (8.314 * ICond->Temp));   // Ni thermal diffusion coefficient, m2/s
  IMaterial->D[2] = (0.78E-4) * std::exp(-231521.6 / (8.314 * ICond->Temp));  // Si thermal diffusion coefficient, m2/s
  IMaterial->DFe = (2.75E-3) * std::exp(-(253968.8) / (8.314 * ICond->Temp)); // Fe thermal diffusion coefficient, m2/s
  ...
  IMaterial->cVol[0] = std::pow(1.093E-9, 3) / 116.; // Atomic volume of T3/G phase, m^-3
  IMaterial->cVol[1] = std::pow(6.67E-10, 3) / 24.;  // Atomic volume of T6/Gamma2 phase,m^-3
  ...
  IProp->DV = 1E-4 * std::exp(-1.3 / (kb * ICond->Temp)); // Vacancy diffusion coefficient, m2/s
